...
function Editor({ initial, member, centerId, onSaved }: { initial: ContactPrefs; member: Member; centerId: string; onSaved: (summary: ContactSummary) => void }) {
  ...
  const [channels, setChannels] = useState<ContactChannel[]>(initial.channelsChosen ? initial.channels : ['sms', 'whatsapp', 'email']);
  const [callTime, setCallTime] = useState<BestCallTime | null>(BEST_CALL_TIMES.find((x) => x === person.best_call_time) ?? null);
